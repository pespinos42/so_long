...
/*   Updated: 2022/12/20 19:40:02 by pespinos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_initialize_textures(t_all *data)
{
	...
	data->i_c_ex = mlx_texture_to_image(data->mlx, data->t_c_ex);
	...
	if (!data->i_co || !data->i_es || !data->i_c_ex)
		exit (EXIT_FAILURE);
...
void	ft_delete_all(t_all *data)
{
	...
	mlx_delete_image(data->mlx, data->i_c_ex);
	...
	mlx_delete_texture(data->t_c_ex);
	...
	while (r < data->rows)
	{
		...
		while (c < data->columns)
		{
			if (data->content[r][c] == '1')
				...
				mlx_image_to_window(data->mlx, data->i_c_ex, 32 * c, 32 * r);
