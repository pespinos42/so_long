...
/*   Updated: 2022/12/22 16:35:48 by pespinos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_enemy(t_all *d)
{
	...
	index = 0;
	find = 0;
	while (index < d->enemies[0].count_enemies)
	{
		if (d->player_row == d->enemies[index].enemy_row && d->player_column == d->enemies[index].enemy_column)
			find = 1;
		index++;
	}
	return (find);
...
int	ft_check_finish(t_all *d/*int r_p, int c_p, t_point_exit *ex*/)
{
	...
	index = 0;
	find = 0;
	while (index < d->exits[0].count_exits)
	{
		if (d->player_row == d->exits[index].exit_row && d->player_column == d->exits[index].exit_column)
			find = 1;
		index++;
	}
	return (find);
...
void	ft_player_position(t_all *d)
{
	...
	find = 0;
	while (d->content[d->player_row] && find == 0)
	{
		d->player_column = 0;
		while (d->content[d->player_row][d->player_column] && find == 0)
		{
			if (d->content[d->player_row][d->player_column] == 'P')
				find = 1;
			if (find == 0)
				d->player_column += 1;
		}
...
int	ft_check_other_position(t_all *d)
{
	...
	ok_d = 1;
	if (d->op_row < 0 || d->op_row >= d->rows)
	{
		printf("ERROR\nFILA INVALIDA\n");
		ok_d = 0;
	}
	if (d->op_column < 0 || d->op_column >= d->columns)
	{
		printf("ERROR\nCOLUMNA INVALIDA");
		ok_d = 0;
	}
	if (ok_d == 1)
	{
		if (d->content[d->op_row][d->op_column] != '1')
		{
			d->content[d->op_row][d->op_column] = 'P';
			if (ft_check_prev_position(d->exits, d->player_row, d->player_column))
				d->content[d->player_row][d->player_column] = 'E';
			else
				d->content[d->player_row][d->player_column] = '0';
			d->player_row = d->op_row;
			d->player_column = d->op_column;
			d->total_moves += 1;
			return (1);
		}
		else
...
void	ft_op_positions(t_all *d)
{
	if (d->move == 'W')
	{
		d->op_row = d->player_row - 1;
		d->op_column = d->player_column;
	}
	else if (d->move == 'A')
	{
		d->op_row = d->player_row;
		d->op_column = d->player_column - 1;
	}
	else if (d->move == 'S')
	{
		d->op_row = d->player_row + 1;
...
void	ft_initialize_data(t_all *d)
{
	...
	d->rows = ft_number_rows(fd);
	fd = open("mapa1.ber", O_RDONLY);
	d->columns = ft_number_columns(fd);
	d->width = d->columns * 32;
	d->height = d->rows * 32;
	fd = open("mapa1.ber", O_RDONLY);
	d->content = ft_create_content(fd, d->rows);
	d->exits = ft_configure_exits(d);
	d->enemies = ft_configure_enemies(d);
	d->total_moves = 0;
}
